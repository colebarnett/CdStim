# -*- coding: utf-8 -*-
"""
Pearce-Hall Model - DYNAMIC ASSOCIABILITY
Parameters: kappa (scaling), eta (associability update rate), beta
Associability (alpha) tracks the smoothed absolute prediction error.
"""
...
def Calc_DistrQlearning_2Targs_PearceHall(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    '''Pearce-Hall Q-learning with dynamic associability'''
    kappa = parameters[0]   # Scaling constant (k)
    eta = parameters[1]     # Associability learning rate (how fast alpha changes)
    ...
    # Initialize Associability (alpha)
    # PH theory assumes novel stimuli have high associability.
    # # Optional allow starting alpha to be a parameter? 
    # SOP - fix it to 1.0 "high initial attention"
    alpha_low = np.ones(len(chosen_target)) 
    alpha_high = np.ones(len(chosen_target))
    ...
    for i in range(len(chosen_target)-1):
        ...
        if chosen_target[i] == 1:
            ...
            # 2. Update Q-Value using CURRENT Associability
            Q_low[i+1] = Q_low[i] + (kappa * alpha_low[i]) * delta
            Q_high[i+1] = Q_high[i]
            
            # 3. Update Associability for NEXT trial
            alpha_low[i+1] = (1 - eta) * alpha_low[i] + eta * abs(delta)
            alpha_high[i+1] = alpha_high[i] # Unchosen options don't change attention?
            
        elif chosen_target[i] == 2:
            delta = float(rewards[i]) - Q_high[i]
            
            Q_high[i+1] = Q_high[i] + (kappa * alpha_high[i]) * delta
            Q_low[i+1] = Q_low[i]
            
            alpha_high[i+1] = (1 - eta) * alpha_high[i] + eta * abs(delta)
            alpha_low[i+1] = alpha_low[i]
...
def loglikelihood_DistrQlearning_2Targs_PearceHall(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
        Calc_DistrQlearning_2Targs_PearceHall(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
    return log_prob_total


def DistrQlearning_2Targs_TwoStage_PearceHall(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
    ...
    print("\n" + "="*60)
    print("MODEL: Pearce-Hall (Dynamic Associability)")
    print("alpha(t+1) = (1-eta)*alpha(t) + eta*|error|")
    print("STAGE 1: Fitting Block A")
    print("="*60)
    
    Q_initial = 0.5 * np.ones(2)
    nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_PearceHall(*args)
    # kappa, eta, beta
    x0 = [0.5, 0.5, 3.0]
    ...
    result_A = op.minimize(nll, x0, 
                           ...
                           method='Nelder-Mead')
    kappa_A, eta_A, beta_A = result_A["x"]
    
    print(f"  kappa (scale): {kappa_A:.4f}, eta (update): {eta_A:.4f}, beta: {beta_A:.4f}")
    
    Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
        Calc_DistrQlearning_2Targs_PearceHall([kappa_A, eta_A, beta_A], 
    ...
    x0_Ap = [kappa_A, eta_A, beta_A]
    
    result_Ap = op.minimize(nll, x0_Ap,
                            args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
                            bounds=[(0,1), (0,1), (0.1,50)],
                            method='Nelder-Mead')
    kappa_Ap, eta_Ap, beta_Ap = result_Ap["x"]
    
    print(f"  kappa (scale): {kappa_Ap:.4f}, eta (update): {eta_Ap:.4f}, beta: {beta_Ap:.4f}")
    print("="*60 + "\n")
    
    Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
        Calc_DistrQlearning_2Targs_PearceHall([kappa_Ap, eta_Ap, beta_Ap], 
    ...
    # Return kappa twice for backwards compatibility
    return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
            kappa_A, kappa_A, None, eta_A, None, None, beta_A,
            kappa_Ap, kappa_Ap, None, eta_Ap, None, None, beta_Ap)