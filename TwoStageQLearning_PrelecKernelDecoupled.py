# -*- coding: utf-8 -*-
"""
Prelec-2 Model + Decoupled Kernel
Parameters: alpha, prelec_gamma, prelec_elevation, ck_weight, ck_decay, softmax_beta
Integrates 2-parameter Prelec weighting with the decoupled choice kernel.
"""
...
def Calc_DistrQlearning_2Targs_PrelecKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    '''Prelec-2 Q-learning + Decoupled Kernel'''
    alpha = parameters[0]
    prelec_gamma = parameters[1]      # Shape
    prelec_elevation = parameters[2]  # Elevation
    ck_weight = parameters[3]
    ck_decay = parameters[4]
    softmax_beta = parameters[5]
    ...
    for i in range(len(chosen_target)-1):
        ...
        # PRELEC-2 & DECOUPLED SOFTMAX
        if instructed_or_freechoice[i+1] == 2:
            # Clip and Distort
            q_low_cl = np.clip(Q_low[i+1], eps, 1-eps)
            q_high_cl = np.clip(Q_high[i+1], eps, 1-eps)
            
            # w(p) = exp( -elevation * (-ln(p))^gamma )
            Q_low_subj = np.exp(-prelec_elevation * (-np.log(q_low_cl)) ** prelec_gamma)
            Q_high_subj = np.exp(-prelec_elevation * (-np.log(q_high_cl)) ** prelec_gamma)
            
            # Decoupled Kernel Bonus
            ck_bonus = ck_weight * (CK_high[i+1] - CK_low[i+1])
            
            # Softmax on Subjective Q
            prob_choice_low[i+1] = 1./(1. + np.exp(softmax_beta*(Q_high_subj - Q_low_subj) + ck_bonus))
...
def loglikelihood_DistrQlearning_2Targs_PrelecKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
        Calc_DistrQlearning_2Targs_PrelecKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
    return log_prob_total


def DistrQlearning_2Targs_TwoStage_PrelecKernelDecoupled(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
    ...
    print("\n" + "="*60)
    print("MODEL: Prelec-2 + Kernel Decoupled")
    print("Subjective Prob Weighting (2-Param) + Decoupled Choice Kernel")
    print("="*60)
    
    Q_initial = 0.5 * np.ones(2)
    nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_PrelecKernelDecoupled(*args)
    # alpha, gamma, elev, ck_weight, ck_decay, beta
    x0 = [0.5, 1.0, 1.0, 0.0, 0.5, 3.0]
    
    result_A = op.minimize(nll, x0, 
                           args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
                           bounds=[(0,1), (0.1, 5), (0.1, 5), (-2,2), (0,1), (0.1,50)],
                           method='Nelder-Mead')
    alpha_A, gamma_A, elev_A, ck_weight_A, ck_decay_A, beta_A = result_A["x"]
    
    print(f"  alpha: {alpha_A:.4f}")
    print(f"  gamma: {gamma_A:.4f}, elev: {elev_A:.4f}")
    ...
    Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
        Calc_DistrQlearning_2Targs_PrelecKernelDecoupled([alpha_A, gamma_A, elev_A, ck_weight_A, ck_decay_A, beta_A], 
    ...
    x0_Ap = [alpha_A, gamma_A, elev_A, ck_weight_A, ck_decay_A, beta_A]
    
    result_Ap = op.minimize(nll, x0_Ap,
                            args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
                            bounds=[(0,1), (0.1, 5), (0.1, 5), (-2,2), (0,1), (0.1,50)],
                            method='Nelder-Mead')
    alpha_Ap, gamma_Ap, elev_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap = result_Ap["x"]
    
    print(f"  alpha: {alpha_Ap:.4f}")
    print(f"  gamma: {gamma_Ap:.4f}, elev: {elev_Ap:.4f}")
    ...
    Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
        Calc_DistrQlearning_2Targs_PrelecKernelDecoupled([alpha_Ap, gamma_Ap, elev_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap], 
    ...
    # Return params
    return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
            alpha_A, gamma_A, elev_A, None, ck_weight_A, ck_decay_A, beta_A,
            alpha_Ap, gamma_Ap, elev_Ap, None, ck_weight_Ap, ck_decay_Ap, beta_Ap)