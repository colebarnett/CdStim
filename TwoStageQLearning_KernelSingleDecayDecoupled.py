# -*- coding: utf-8 -*-
"""
DECOUPLED Choice Kernel + Decay Model - SINGLE-ALPHA, WITH DECAY, NO LAPSE
...
def Calc_DistrQlearning_2Targs_KernelSingleDecayDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
...
def loglikelihood_DistrQlearning_2Targs_KernelSingleDecayDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
        Calc_DistrQlearning_2Targs_KernelSingleDecayDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
    return log_prob_total


def DistrQlearning_2Targs_TwoStage_KernelSingleDecayDecoupled(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
    ...
    print("\n" + "="*60)
    print("MODEL: KernelSingleDecay Decoupled (Single-Alpha + Decay, No Lapse)")
    print("STAGE 1: Fitting Block A")
    print("="*60)
    
    Q_initial = 0.5 * np.ones(2)
    nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_KernelSingleDecayDecoupled(*args)
    ...
    Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
        Calc_DistrQlearning_2Targs_KernelSingleDecayDecoupled([alpha_A, decay_A, ck_weight_A, ck_decay_A, beta_A], 
    ...
    Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
        Calc_DistrQlearning_2Targs_KernelSingleDecayDecoupled([alpha_Ap, decay_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap], 
