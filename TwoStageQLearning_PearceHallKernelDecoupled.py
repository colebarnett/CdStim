# -*- coding: utf-8 -*-
"""
Pearce-Hall Model + Decoupled Kernel
Parameters: kappa, eta, ck_weight, ck_decay, beta
Integrates dynamic associability with the decoupled choice kernel.
"""
...
def Calc_DistrQlearning_2Targs_PearceHallKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    '''Pearce-Hall + Decoupled Kernel'''
    kappa = parameters[0]   # Scaling constant
    eta = parameters[1]     # Associability update rate
    ...
    for i in range(len(chosen_target)-1):
        ...
        # Softmax Choice (DECOUPLED)
        if instructed_or_freechoice[i+1] == 2:
            ck_bonus = ck_weight * (CK_high[i+1] - CK_low[i+1])
            
            prob_choice_low[i+1] = 1./(1. + np.exp(beta*(Q_high[i+1] - Q_low[i+1]) + ck_bonus))
...
def loglikelihood_DistrQlearning_2Targs_PearceHallKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
        Calc_DistrQlearning_2Targs_PearceHallKernelDecoupled(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
    return log_prob_total


def DistrQlearning_2Targs_TwoStage_PearceHallKernelDecoupled(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
    ...
    print("\n" + "="*60)
    print("MODEL: Pearce-Hall + Kernel Decoupled")
    print("Dynamic Associability + Decoupled Choice Kernel")
    print("="*60)
    
    Q_initial = 0.5 * np.ones(2)
    nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_PearceHallKernelDecoupled(*args)
    # kappa, eta, ck_weight, ck_decay, beta
    x0 = [0.5, 0.5, 0.0, 0.5, 3.0]
    
    result_A = op.minimize(nll, x0, 
                           args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
                           bounds=[(0,1), (0,1), (-2,2), (0,1), (0.1,50)],
                           method='Nelder-Mead')
    kappa_A, eta_A, ck_weight_A, ck_decay_A, beta_A = result_A["x"]
    
    print(f"  kappa: {kappa_A:.4f}, eta: {eta_A:.4f}")
    ...
    Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
        Calc_DistrQlearning_2Targs_PearceHallKernelDecoupled([kappa_A, eta_A, ck_weight_A, ck_decay_A, beta_A], 
    ...
    x0_Ap = [kappa_A, eta_A, ck_weight_A, ck_decay_A, beta_A]
    
    result_Ap = op.minimize(nll, x0_Ap,
                            args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
                            bounds=[(0,1), (0,1), (-2,2), (0,1), (0.1,50)],
                            method='Nelder-Mead')
    kappa_Ap, eta_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap = result_Ap["x"]
    
    print(f"  kappa: {kappa_Ap:.4f}, eta: {eta_Ap:.4f}")
    ...
    Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
        Calc_DistrQlearning_2Targs_PearceHallKernelDecoupled([kappa_Ap, eta_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap], 
    ...
    # Return params
    return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
            kappa_A, kappa_A, None, eta_A, ck_weight_A, ck_decay_A, beta_A,
            kappa_Ap, kappa_Ap, None, eta_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap)