...
def GetIndexes(behavior,subject,block,choice,context,trial_type,rewarded,stayshift,epoch,debug):
    ...
    if subject == 'Luigi':
        ...
        raise ValueError("Not a valid key for subject")
    ...
    if rewarded == 'Rewarded':
        ...
        raise ValueError("Not a valid key for rewarded")
    ...
    if stayshift == 'Win-stay':
        ...
        raise ValueError("Not a valid key for stayshift")
    ...
    if block == 'BlA':
        ...
        raise ValueError("Not a valid key for block")
...
class SpectralPowerAnalysis_TwoAreas():
    ...
    def __init__(self,subject,areas,freq_band_names,freq_bands,stim_or_sham,
        ...
        self.LFPfolderpath = os.path.join(data_path,self.subject,"LFP\\")
...
#             ax.set_aspect(1.6)
    ...
    def PlotSpectsDiff(self,compObj,t1,t2,f1,f2,v1,v2, doylabelFlag,doColorbarFlag,alldecoratorsFlag):
        '''
        Plots the session averaged spectrograms for both areas
        ...
        spects = self.spects_sessavg - compObj.spects_sessavg    
...
#                             print(area1_ch,'&',area2_ch)
                    ...
                    #t = np.array(connectivity.times) #cuz errror -> + np.min(session_data['t']) # the +min(t) is to align new t vector with original t vector
...
#                     rank_size = 1
                    ...
                    t = np.array(connectivity.times) + np.min(session_data['t']) # the +min(t) is to align new t vector with original t vector
                    #t = np.array(connectivity.times)  #cuz error -> + np.min(session_data['t']) # the +min(t) is to align new t vector with original t vector
                    f = np.array(connectivity.freqs)
    ...
    def GetGC_RepSession(self,LFP_path,file_name,behavior_file,overwriteFlag):
        ...
        if not overwriteFlag and os.path.isfile(file_load_name):
            ...
            with open(os.path.join(data_path, self.subject, "Behavior", behavior_file),'rb') as f_:
                behavior = pickle.load(f_)
            print(f'{file_name} loaded.')
