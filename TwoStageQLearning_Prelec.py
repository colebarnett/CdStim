# -*- coding: utf-8 -*-
"""
Prelec-2 Model - SUBJECTIVE PROBABILITY WEIGHTING (2 Parameters)
Parameters: alpha, prelec_gamma (shape), prelec_elevation, softmax_beta
Equation: w(p) = exp( -prelec_elevation * (-ln(p))^prelec_gamma )
"""
...
def Calc_DistrQlearning_2Targs_Prelec(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    '''Q-learning with 2-Parameter Prelec weighting on Q-values'''
    alpha = parameters[0]
    prelec_gamma = parameters[1]      # Shape/Curvature
    prelec_elevation = parameters[2]  # Elevation (Optimism/Pessimism)
    softmax_beta = parameters[3]      # Inverse Temperature
    ...
    for i in range(len(chosen_target)-1):
        ...
        if instructed_or_freechoice[i+1] == 2:
            # Clip Q to valid probability range (0,1)
            q_low_cl = np.clip(Q_low[i+1], eps, 1-eps)
            q_high_cl = np.clip(Q_high[i+1], eps, 1-eps)
            
            # Apply Prelec-2: w(p) = exp( -elevation * (-ln(p))^gamma )
            ...
            # Softmax on SUBJECTIVE values
            prob_choice_low[i+1] = 1./(1. + np.exp(softmax_beta*(Q_high_subj - Q_low_subj)))
...
def loglikelihood_DistrQlearning_2Targs_Prelec(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
    Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
        Calc_DistrQlearning_2Targs_Prelec(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
    return log_prob_total


def DistrQlearning_2Targs_TwoStage_Prelec(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
    ...
    print("\n" + "="*60)
    print("MODEL: Prelec-2 (Shape + Elevation)")
    print("w(p) = exp( -elevation * (-ln(p))^gamma )")
    print("STAGE 1: Fitting Block A")
    print("="*60)
    
    Q_initial = 0.5 * np.ones(2)
    nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_Prelec(*args)
    # alpha, gamma, elevation, beta
    x0 = [0.5, 1.0, 1.0, 3.0]
    
    # Bounds: 
    # Gamma: 0.1 to 5 (Curvature)
    # Elevation: 0.1 to 5 (1=Neutral, <1 Optimism, >1 Pessimism)
    result_A = op.minimize(nll, x0, 
                           args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
                           bounds=[(0,1), (0.1, 5), (0.1, 5), (0.1,50)], 
                           method='Nelder-Mead')
    alpha_A, gamma_A, elev_A, beta_A = result_A["x"]
    
    print(f"  alpha: {alpha_A:.4f}")
    print(f"  gamma (shape): {gamma_A:.4f}, elev (optimism): {elev_A:.4f}")
    print(f"  softmax_beta: {beta_A:.4f}")
    
    Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
        Calc_DistrQlearning_2Targs_Prelec([alpha_A, gamma_A, elev_A, beta_A], 
    ...
    x0_Ap = [alpha_A, gamma_A, elev_A, beta_A]
    
    result_Ap = op.minimize(nll, x0_Ap,
                            args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
                            bounds=[(0,1), (0.1, 5), (0.1, 5), (0.1,50)],
                            method='Nelder-Mead')
    alpha_Ap, gamma_Ap, elev_Ap, beta_Ap = result_Ap["x"]
    
    print(f"  alpha: {alpha_Ap:.4f}")
    print(f"  gamma (shape): {gamma_Ap:.4f}, elev (optimism): {elev_Ap:.4f}")
    print(f"  softmax_beta: {beta_Ap:.4f}")
    print("="*60 + "\n")
    
    Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
        Calc_DistrQlearning_2Targs_Prelec([alpha_Ap, gamma_Ap, elev_Ap, beta_Ap], 
    ...
    # Return 4 params per block
    return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
            alpha_A, gamma_A, elev_A, None, None, None, beta_A,
            alpha_Ap, gamma_Ap, elev_Ap, None, None, None, beta_Ap)